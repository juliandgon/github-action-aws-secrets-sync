...
python3 ${SCRIPT_DIR}/sync-with-secretsmanager.py --secret_arn ${AWS_TGT_SECRETS_ARN} --secret_json_file ${SECRETS_CONFIG_FILE} --exclude_filter_match="${EXCLUDE_FILTER}" --include_filter_match="${INCLUDE_FILTER}" --remove_pattern="${REMOVE_PATTERN}"